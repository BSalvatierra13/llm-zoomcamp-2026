...
def logfire_source(
    ...
    """Load trace/span data from the Pydantic Logfire Query API.

    Args:
        access_token: Logfire read token (Bearer auth). Auto-loaded from secrets.toml.
        base_url: Logfire API base URL, e.g. "https://logfire-us.pydantic.dev"
            (use "https://logfire-eu.pydantic.dev" for EU-region projects).
            Auto-loaded from config.toml.

    Example:
        pipeline.run(logfire_source())
        pipeline.run(logfire_source(base_url="https://logfire-eu.pydantic.dev"))
    """
    config: RESTAPIConfig = {
        "client": {
            "base_url": base_url,
            ...
            # without this, Logfire returns Apache Arrow bytes instead of JSON
            # and dlt's response.json() parsing fails
        ...
        "resources": [
            {
                ...
                "endpoint": {
                    ...
                    "json": {
                        ...
                        "min_timestamp": pendulum.now("UTC")
                        .subtract(days=1)
                        .to_iso8601_string(),
                        "limit": 100,
                    },
                    # the query endpoint returns {"schema": {...}, "data": [...]}
                    # with no pagination metadata - treat it as a single page
                    "data_selector": "data",
